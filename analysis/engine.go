...
// processData 从 SwitchChannel 读取数据并执行分析
func processData() {
	for handbandData := range data.SwitchChannel {
...
func checkHeartRate(d data.BandData) {
...
func checkAcceleration(d data.BandData) {
	// 计算加速度合向量的平方
...
func triggerAlert(driverID, alertType, message string) {
	...
	// 检查是否在静默期内
	if time.Since(lastAlertTime) < config.Cfg.Analysis.AlertCooldown {
